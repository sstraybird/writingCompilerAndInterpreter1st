...
TYPE_STRUCT_PTR exec_variable(SYMTAB_NODE_PTR idp, USE use)
{
	...
	*/
	...
	if ((use != TARGET_USE) && (use != VARPARM_USE))
		trace_data_fetch(idp, tp, (tp->form == ARRAY_FORM) || (tp->form == RECORD_FORM) ? tos->address : tos);
...
promote_operands_to_real(STACK_ITEM_PTR operandp1, TYPE_STRUCT_PTR tp1, STACK_ITEM_PTR operandp2, TYPE_STRUCT_PTR tp2)
{
