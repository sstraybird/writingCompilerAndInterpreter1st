...
read_readln(SYMTAB_NODE_PTR rtn_idp)
{
    TYPE_STRUCT_PTR actual_parm_tp;
...
/*                              <expr> : <expr> : <expr>        */
/*--------------------------------------------------------------*/
write_writeln(SYMTAB_NODE_PTR rtn_idp)
{
    ...
    if (token == LPAREN)
    {
        do
        {
            ...
            if ((actual_parm_tp->form != SCALAR_FORM) && (actual_parm_tp != boolean_typep) && ((actual_parm_tp->form != ARRAY_FORM) || (actual_parm_tp->info.array.elmt_typep != char_typep)))
...
TYPE_STRUCT_PTR ord()
{
    ...
    if (token == LPAREN)
    {
        get_token();
        parm_tp = base_type(expression());
        if ((parm_tp->form != ENUM_FORM) && (parm_tp != char_typep))
            error(INCOMPATIBLE_TYPES);
        if_token_get_else_error(RPAREN, MISSING_RPAREN);
    }
    else
