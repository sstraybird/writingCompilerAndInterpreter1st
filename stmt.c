...
assignment_statement(SYMTAB_NODE_PTR var_idp)
{
...
case_branch(TYPE_STRUCT_PTR expr_tp)
{
    ...
    case_itemp = old_case_item_tail == NULL ? case_item_head : old_case_item_tail->next;
    while (case_itemp != NULL)
    {
...
TYPE_STRUCT_PTR case_label()
{
 ...
 //           error(INVALID_CONSTANT);
        if (literal.type == INTEGER_LIT) {
            case_itemp->label_value = sign == PLUS ? literal.value.integer : -literal.value.integer;
        }
        else {
            error(INVALID_CONSTANT);
        }
